...
def _hourly_results_to_dataframe(results) -> pd.DataFrame:
    ...
    df["Puissance prechauffage HT solaire (kW)"] = df["solar_ht_from_buffer_kwh"]
    ...
    df["Puissance chaleur utile totale (kW)"] = df["solar_ht_from_buffer_kwh"] + df["heat_bt_from_pac_kwh"]
