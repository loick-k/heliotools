...
```text
Q_solaire_BTES_brut =
    G_tilt_h * A_capteurs * eta_capteur_BTES * eta_systeme
```
...
| `Hypothèses!D13` | aide énergie, â‚¬/MWh.an |
| `X17` | CAPEX brut solaire, â‚¬ |
| `X19` | autres aides publiques déjà acquises, â‚¬ |
...
### Prudence sur l'interprétation BTES

Miceli et al. 2026 rappellent qu'une approche g-function peut rester pertinente pour simuler rapidement des champs de
sondes sur de longues durées, mais que la gestion de l'historique des charges devient un sujet numérique important quand
il existe des alternances entre injection solaire et extraction PAC.

HelioStock conserve le backend `pygfunction` pour le calcul de production. Le modèle reste un outil de prédimensionnement
d'opportunité : il ne doit pas être assimilé à un jumeau numérique de BTES intersaisonnier.

Pour éviter une interprétation excessive, les résultats utilisateur restent centrés sur des grandeurs directement
exploitables : énergie solaire injectée, énergie extraite par la PAC, températures source/paroi, heures sous seuil, critère
GMI et trajectoire annuelle du champ. Les indicateurs avancés propres aux modèles BTES détaillés ne sont pas présentés
comme KPI métier à ce stade.

`pygfunction` ne modélise pas explicitement l'isolation supérieure d'un vrai BTES, les conditions aux limites complexes,
l'hétérogénéité verticale fine, l'hydraulique MIFT série/parallèle détaillée ni l'inertie court terme tube/coulis. HelioStock
reste donc un outil de prédimensionnement d'opportunité, pas un modèle détaillé de stockage intersaisonnier.
