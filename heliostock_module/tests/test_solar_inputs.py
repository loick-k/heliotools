...
def test_solar_inputs_do_not_reactivate_legacy_daily_buffer_loss_fraction():
    ...
    assert collector.daily_buffer_model == "stratified_3_nodes"


def test_solar_inputs_reject_legacy_mixed_tank_model():
    solar = SolarInputs(
        area_m2=100.0,
        eta0=0.8,
        a1_w_m2_k=3.0,
        ...
        solar_buffer_hx_approach_k=5.0,
        solar_buffer_collector_approach_k=10.0,
        daily_buffer_model="mixed",
    )

    assert "stratified_3_nodes" in " ".join(solar.validate())
...
def test_solar_ht_direct_is_only_a_legacy_alias_name():
    ...
    assert "HelioDyn utilise uniquement le ballon solaire stratifié 3 nœuds" in source
    assert "if stratified_tank is not None" not in source
